...
import {deletePosts, likePosts} from "../../../redux/postsSlice";
        ...
        <Card className={classes.card} raised elevation={6}>
            ...
            <CardActions className={classes.cardActions}>
                <Button size={'small'} color={'primary'} disabled={!user?.result}
                        onClick={() => dispatch(likePosts({currentId}))}>
                    <Likes/>
                </Button>
                {(user?.result?.googleId === post?.creator || user?.result?._id === post?.creator) && (
                    <Button size={'small'} color={'primary'} onClick={() => dispatch(deletePosts({currentId}))}>
